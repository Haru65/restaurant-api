...
import { Pool } from "pg";
import { ensureUnifiedSuperadminSchema, handleUnifiedSuperadmin } from "./unified-superadmin.mjs";
...
const initializeDatabase = async () => {
  ...
  await ensureUnifiedSuperadminSchema(pool);
  await pool.query(
    `DO $$
     BEGIN
       IF NOT EXISTS (
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (await handleUnifiedSuperadmin({
      req,
      res,
      url,
      pathName,
