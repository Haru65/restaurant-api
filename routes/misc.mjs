...
router.get('/crm/customers', authenticate, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT * FROM crm_customers WHERE ($1::int IS NULL OR restaurant_id=$1) ORDER BY name`,
...
router.post('/crm/customers', authenticate, async (req, res) => {
  try {
    const { name, email, phone, visits, totalSpent, vip, restaurantId } = req.body;
    ...
    if (!targetRestaurantId) return res.status(400).json({ error: 'restaurantId required' });
    const { rows } = await query(
      `INSERT INTO crm_customers (restaurant_id, name, email, phone, visits, total_spent, vip) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [targetRestaurantId, name.trim(), email.trim(), phone.trim(), Number(visits || 1), Number(totalSpent || 0), Boolean(vip)]
    );
    res.status(201).json(rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.patch('/crm/customers/:id', authenticate, async (req, res) => {
  try {
    ...
    const { rows } = await query(
      ...
      [name, email, phone, visits !== undefined ? Number(visits) : null, spent, vip !== undefined ? Boolean(vip) : null, req.params.id, scopedRestaurantId(req)]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Not found' });
    res.json(rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
router.delete('/crm/customers/:id', authenticate, async (req, res) => {
  try {
    const result = await query(
      `DELETE FROM crm_customers WHERE id=$1 AND ($2::int IS NULL OR restaurant_id=$2)`,
...
router.get('/reports/overview', authenticate, async (req, res) => {
  try {
    const rid = scopedRestaurantId(req);
    const [ordersRes, customersRes] = await Promise.all([
      query(`SELECT total, items FROM orders WHERE ($1::int IS NULL OR restaurant_id=$1)`, [rid]),
      query(`SELECT COUNT(*) as count FROM crm_customers WHERE ($1::int IS NULL OR restaurant_id=$1)`, [rid]),
    ]);
...
router.get('/delivery-api-keys', authenticate, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT swiggy_key, zomato_key FROM delivery_api_keys WHERE restaurant_id=$1`,
      [req.user.restaurantId]
    );
    res.json({ swiggy: rows[0]?.swiggy_key || '', zomato: rows[0]?.zomato_key || '' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
router.put('/delivery-api-keys', authenticate, async (req, res) => {
  try {
    const swiggy = req.body.swiggy || '';
    const zomato = req.body.zomato || '';
    await query(
      `INSERT INTO delivery_api_keys (restaurant_id, swiggy_key, zomato_key)
       VALUES ($1,$2,$3)
       ON CONFLICT (restaurant_id) DO UPDATE SET swiggy_key=EXCLUDED.swiggy_key, zomato_key=EXCLUDED.zomato_key, updated_at=NOW()`,
      [req.user.restaurantId, swiggy, zomato]
    );
