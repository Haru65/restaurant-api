...
router.post('/auth/login', async (req, res) => {
  try {
    ...
    if (user.role !== 'superadmin' && !user.restaurant_id) {
      return res.status(403).json({ error: 'Your account is not assigned to a tenant. Please contact support.' });
    }
...
router.post('/auth/change-password', async (req, res) => {
  try {
    ...
    await query(
      `UPDATE users SET password_hash=$1, must_change_password=FALSE, temp_password=NULL WHERE id=$2`,
      [await bcrypt.hash(newPassword, 10), decoded.id]
    );
