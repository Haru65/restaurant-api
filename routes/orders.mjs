...
const getPrinterService = async () => {
  ...
  try {
    ...
    console.warn('Printer service not available:', err.message);
    printerService = {
...
router.get('/orders', authenticate, async (req, res) => {
  try {
    const restaurantId = req.user.role === 'superadmin' ? null : Number(req.user.restaurantId) || -1;
    const { rows } = await query(
      `SELECT o.*, COALESCE(t.section, 'Main Hall') as section FROM orders o
       ...
       WHERE ($1::int IS NULL OR o.restaurant_id=$1) ORDER BY o.created_at DESC`,
      [restaurantId]
    );
    res.json(rows.map(o => ({
      ...o,
...
router.post(['/orders/print-kot', '/order/print-kot'], authenticate, async (req, res) => {
  try {
