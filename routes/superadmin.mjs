...
const formatTicket = (row) => ({
  ...row,
  ticket: row.ticket || `TKT-${String(row.id).padStart(4, '0')}`,
  priority: row.priority || 'medium',
  assignee: row.assignee || 'Unassigned',
  sla: row.status === 'resolved' ? 'Met' : row.sla || '24h left',
  time: row.created_at ? new Date(row.created_at).toISOString() : null
});
const formatSetting = (row) => ({ ...row, key: row.setting_key });
...
router.post('/superadmin/support', ...sa, async (req, res) => {
  try {
    const { subject, restaurant, status, priority, assignee, sla } = req.body;
    if (!subject) return res.status(400).json({ error: 'Subject required' });
    const { rows } = await query(
      `INSERT INTO support_tickets (subject, restaurant, status, priority, assignee, sla) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [subject.trim(), restaurant || '', status || 'open', priority || 'medium', assignee || 'Unassigned', sla || '24h left']
    );
  ...
  try {
    ...
    const { rows } = await query(
      `UPDATE support_tickets SET subject=COALESCE($1,subject), restaurant=COALESCE($2,restaurant), status=COALESCE($3,status),
       priority=COALESCE($4,priority), assignee=COALESCE($5,assignee), sla=COALESCE($6,sla)
       WHERE id=$7 RETURNING *`,
      [subject, restaurant, status, priority, assignee, sla, req.params.id]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Not found' });
...
router.get('/superadmin/settings', ...sa, async (req, res) => {
  try {
    ...
    res.json(rows.map(formatSetting));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
